%function to search for the centre coordinates of the pupil and the iris along with their radii
...
%Once the iris has been detected;the pupil's centre coordinates
...
for k=1:s 
    if (X(k)>rmin)&(Y(k)>rmin)&(X(k)<=(rows-rmin))&(Y(k)<(cols-rmin))
            ...
            M=min(min(A));
            %this process scans the neighbourhcood of the selected pixel
...
cp=search(I,round(0.1*r),round(0.8*r),ci(1)*scale,ci(2)*scale,'pupil');%biological limits on the relative sizes of the iris and pupil
cp=cp/scale;
